...
public:
    ...
    //Function that moves the nth element of the queue to the front.
...
{
    if (isEmptyQueue()) {
        cout << "Cannot move element from an empty queue." << endl;
        return;
    }
    ...
    // Move the nth element to the front
    int index = (queueFront + n - 1) % maxQueueSize;
    Type element = list[index];
    for (int i = index; i > queueFront; i = (i - 1 + maxQueueSize) % maxQueueSize)
        list[i] = list[(i - 1 + maxQueueSize) % maxQueueSize];
    list[queueFront] = element;
}
#endif
